...
function causesBust(state: GameState, p: Player, card: Card): boolean {
  if (card.type === 'Dynamit') {
    return true; // Dynamit bustet jetzt immer
  }
...
function handleBust(state: GameState, card: Card): GameState {
  ...
  // Add the card normally so it flies in
...
function applyFluchCancel(state: GameState, card: Card): GameState {
  ...
  // Wir wechseln in eine spezielle Phase, damit das UI Zeit hat, die Animation zu zeigen.
...
function addToDisplay(state: GameState, card: Card, opts?: { isSecret?: boolean }): GameState {
  ...
  if (opts?.isSecret !== undefined) {
      newItem.isSecret = opts.isSecret;
  }
  ...
  switch (action.type) {
    ...
    case 'HINTERHALT_TARGET': {
        ...
        if (isCageTarget && targetItem.cagedBy) {
            newDisplay = targetPlayer.display.map(i => {
                if (i.id === targetId) {
                    const newI = { ...i };
                    delete newI.cagedBy;
                    return newI;
            ...
            // Create a temporary display item for the destroyed cage card
            const destroyedCageItem: DisplayItem = {
                id: `hinterhalt-destroyed-${targetItem.cagedBy.id}`,
                card: targetItem.cagedBy,
                isHinterhaltDestroyed: true
            };
            newDisplay.push(destroyedCageItem);
            
            const newlyUncoveredType = targetItem.card.type;
            const activeOfThisType = newDisplay.filter(i => !i.cagedBy && !i.isSecret && !i.isBusted && !i.isHinterhaltDestroyed && i.card.type === newlyUncoveredType).length;
            
            if (activeOfThisType > 1) {
                didBust = true;
                const armorIndex = newDisplay.findIndex(i => !i.cagedBy && !i.isSecret && !i.isBusted && !i.isHinterhaltDestroyed && i.card.type === 'Schrottrüstung');
                if (armorIndex !== -1) {
                    const keptItems = newDisplay.slice(0, armorIndex);
                    const lostItems = newDisplay.slice(armorIndex).map(i => i.isHinterhaltDestroyed ? i : { ...i, isBusted: true });
                    ...
                    newDisplay = newDisplay.map(i => i.isHinterhaltDestroyed ? i : { ...i, isBusted: true });
    ...
    case 'CLEANUP_HINTERHALT': {
        ...
        ns.players = ns.players.map(p => ({
            ...p,
            display: p.display.filter(i => !i.isHinterhaltDestroyed)
        }));
        return advanceTurn(ns);
    }
